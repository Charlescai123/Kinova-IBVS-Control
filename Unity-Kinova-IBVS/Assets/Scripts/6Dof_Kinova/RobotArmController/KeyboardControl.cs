...
public class KeyboardControl : MonoBehaviour
{
    ...
    //            Debug.Log("New Joints are:");
    //            for (int i = 0; i < newJoints.Length; i++)
    //            {
    //                Debug.Log(newJoints[i]);
    //            }
    //            MoveAllJoints(newJoints);
    //        }
    //    }
    ...
    //public void JointGo(float[] q)
    //{
    //    for (int i = 0; i < q.Length; i++)
    //    {
    //        var wtf = articulationChain[i].xDrive;
    //        wtf.target = q[i];
    //        articulationChain[i].xDrive = wtf;
    //    }
    //}
    ...
    {
        deltaPosition = Vector3.zero;
        float delta = 0.0025f; // 0.5 sample rate
        deltaPosition += new Vector3(0, 0, -delta);
        ...
        if (deltaPosition != Vector3.zero)
        {
            ...
            {
                var wtf = articulationChain[i].xDrive;
                newJoints[i] *= Mathf.Rad2Deg;
                wtf.target = newJoints[i];
                articulationChain[i].xDrive = wtf;
                Debug.Log(newJoints[i]);
            }
